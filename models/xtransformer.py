...
class XTransformer(BasicModel):
    def __init__(self):
        ...
        geo_sequential = []
        geo_sequential.append(nn.Linear(cfg.MODEL.GEO_DIM, cfg.MODEL.ATT_FEATS_EMBED_DIM))
        #geo_sequential.append(
        #    utils.activation(cfg.MODEL.ATT_FEATS_EMBED_ACT))  # Assuming you want to use the same activation function

        # Optional Dropout for geo_feats
        if cfg.MODEL.DROPOUT_GEO_FEAT > 0:
            geo_sequential.append(nn.Dropout(cfg.MODEL.DROPOUT_GEO_FEAT))

        # Create the geo_feats embedding sequential layer
        self.geo_embed = nn.Sequential(*geo_sequential) if len(geo_sequential) > 0 else None

        # Add GeoFeature Transformation Layer
        #self.geo_dropout = nn.Dropout(cfg.MODEL.DROPOUT_GEO_FEAT)  # Optional dropout for geo_feats
        #self.geo_embed = nn.Linear(cfg.MODEL.GEO_DIM, cfg.MODEL.ATT_FEATS_EMBED_DIM)

        # Add GeoAttention Layer
        self.geo_attention = GeoAttention(cfg.MODEL.ATT_FEATS_EMBED_DIM)

        self.encoder = Encoder(
            embed_dim=cfg.MODEL.BILINEAR.DIM,
            geo_dim=cfg.MODEL.GEO_DIM,
    ...
    def forward(self, **kwargs):
        ...
        # Embed attention features
        att_feats = self.att_embed(att_feats)

        # Apply Geo_dropout, Embed, GeoAttention
        geo_feats = self.geo_embed(geo_feats)
        #geo_feats = self.geo_dropout(geo_feats)
        att_feats = self.geo_attention(att_feats, geo_feats)

        # Pass through encoder
        gx, encoder_out = self.encoder(att_feats, att_mask, geo_feats)

        # Pass through decoder
        decoder_out = self.decoder(gx, seq, encoder_out, att_mask, seq_mask, geo_feats)
        return decoder_out
    ...
    def get_logprobs_state(self, **kwargs):
        ...
        geo_feats = kwargs[cfg.PARAM.GEO_FEATS]  # Extract geo_feats from kwargs
        ...
        seq_mask = subsequent_mask(ys.size(1)).to(encoder_out.device).type(torch.cuda.FloatTensor)[:, -1, :].unsqueeze(
            1)
        decoder_out = self.decoder(gx, ys[:, -1].unsqueeze(-1), encoder_out, att_mask, seq_mask, geo_feats, p_att_feats,
                                   precompute=True).squeeze(1)
    ...
    def decode_beam(self, **kwargs):
        ...
        for t in range(cfg.MODEL.SEQ_LEN):
            ...
            if t == 0:
                ...
                geo_feats = utils.expand_tensor(geo_feats, beam_size)  # Expand geo_feats for beam size
                ...
                kwargs[cfg.PARAM.GEO_FEATS] = geo_feats  # Include geo_feats in kwargs
    ...
    def decode(self, **kwargs):
        ...
        att_feats = kwargs[cfg.PARAM.ATT_FEATS]
        att_mask = kwargs[cfg.PARAM.ATT_FEATS_MASK]
        geo_feats = kwargs[cfg.PARAM.GEO_FEATS]  # Add geo_feats
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.geo_attention = GeoAttention(embed_dim)  # Dual attention for geo

    def forward(self, x, mask, geo_feats):  # Add geo_feats
        x = self.geo_attention(x, geo_feats)  # Apply geometric attention
        gx = (torch.sum(x * mask.unsqueeze(-1), 1) / torch.sum(mask.unsqueeze(-1), 1))

        gx_arr = [gx]
        for layer in self.layers:
            gx, x = layer(gx, x, mask, geo_feats)  # Pass geo_feats to each layer
            gx_arr.append(gx)
...
class EncoderLayer(nn.Module):
    def __init__(
        ...
        #self.geo_dropout = nn.Dropout(cfg.MODEL.DROPOUT_GEO_FEAT)  # Apply dropout based on the model configuration
        #self.geo_embed = nn.Linear(geo_dim, embed_dim)  # Geo feats transform
        self.geo_attention = GeoAttention(embed_dim)  # Dual attention for geo
    ...
    def forward(self, gx, x, mask, geo_feats):  # Add geo_feats as a parameter
        #geo_feats = self.geo_dropout(geo_feats)  # Apply dropout after transformation
        #geo_feats = self.geo_embed(geo_feats)
        x = self.geo_attention(x, geo_feats)  # Apply geometric attention
...
class Decoder(nn.Module):
    def __init__(
        ...
        for i in range(layer_num):
            sublayer = DecoderLayer(
                embed_dim=embed_dim,
                geo_dim=geo_dim,  # Pass geo_dim to DecoderLayer
                dropout=dropout,
                att_type=att_type,
                att_heads=att_heads,
                att_mid_dim=att_mid_dim,
    ...
    def forward(self, gx, prev_output_tokens, encoder_out, att_mask, seq_mask=None, geo_feats=None, p_att_feats=None, precompute=False):
        ...
        for layerid, layer in enumerate(self.layers):
            ...
            gx, x = layer(gx, x, encoder_out, att_mask, seq_mask=seq_mask, p_key=p_key, p_value2=p_value2,
                          geo_feats=geo_feats, precompute=precompute)  # Pass geo_feats to each layer
            gx_arr.append(gx)
...
class DecoderLayer(nn.Module):
    def __init__(
        ...
        self.word_attn = LowRank(
            embed_dim=embed_dim,
            geo_dim=geo_dim,  # Add geo_dim for geometric attention
            att_type=att_type,
            att_heads=att_heads,
            att_mid_dim=att_mid_dim,
            att_mid_drop=att_mid_drop)
        ...
        self.cross_att = LowRank(
            embed_dim=embed_dim,
            geo_dim=geo_dim,  # Add geo_dim for cross attention
            att_type=att_type,
            att_heads=att_heads,
            att_mid_dim=att_mid_dim,
            att_mid_drop=att_mid_drop)
    ...
    def forward(
            ...
            att_mask,
            seq_mask,
            geo_feats,  # Add geo_feats as a parameter
            p_key=None,
            p_value2=None,
            precompute=False
    ):
