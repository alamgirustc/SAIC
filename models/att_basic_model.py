...
class AttBasicModel(BasicModel):
    def __init__(self):
        ...
        sequential = []
        if cfg.MODEL.ATT_FEATS_EMBED_DIM > 0:
            sequential.append(nn.Linear(6, cfg.MODEL.ATT_FEATS_EMBED_DIM))
        sequential.append(utils.activation(cfg.MODEL.ATT_FEATS_EMBED_ACT))
        if cfg.MODEL.DROPOUT_ATT_EMBED > 0:
            sequential.append(nn.Dropout(cfg.MODEL.DROPOUT_ATT_EMBED))
        if cfg.MODEL.ATT_FEATS_NORM == True:
            sequential.append(torch.nn.LayerNorm(cfg.MODEL.ATT_FEATS_EMBED_DIM))
        self.geo_embed = nn.Sequential(*sequential) if len(sequential) > 0 else None
        ...
        if cfg.MODEL.ATT_FEATS_NORM == True:
            sequential.append(torch.nn.LayerNorm(cfg.MODEL.ATT_FEATS_EMBED_DIM))
        self.add_embed = nn.Sequential(*sequential) if len(sequential) > 0 else None

        # geometric features dropout
        #self.geo_feat_dropout = nn.Dropout(cfg.MODEL.DROPOUT_GEO_FEAT)

        # Additional features dropout
        self.add_feat_dropout = nn.Dropout(cfg.MODEL.DROPOUT_ADD_FEAT)
    ...
    def make_kwargs(self, wt, gv_feat, att_feats, att_mask, p_att_feats, state,  geo_feats, add_feats, **kgs):
        kwargs = kgs
        ...
        kwargs[cfg.PARAM.ADD_FEATS] = add_feats
    ...
    def preprocess(self, **kwargs):
        ...
        geo_feats = kwargs.get(cfg.PARAM.GEO_FEATS, None)
        add_feats = kwargs.get(cfg.PARAM.ADD_FEATS, None)
        ...
        #    add_feats = self.add_feat_dropout(add_feats)

        # bilinear
        if cfg.MODEL.BILINEAR.DIM > 0:
            gv_feat, att_feats = self.encoder_layers(gv_feat, att_feats, att_mask, geo_feats=geo_feats, add_feats=add_feats)
            keys, value2s = self.attention.precompute(att_feats, att_feats, geo_feats=geo_feats, add_feats=add_feats)
            p_att_feats = torch.cat([keys, value2s], dim=-1)

        return gv_feat, att_feats, att_mask, p_att_feats, geo_feats, add_feats

    # gv_feat -- batch_size * cfg.MODEL.GVFEAT_DIM
    # att_feats -- batch_size * att_num * att_feats_dim
    def forward(self, **kwargs):
        seq = kwargs[cfg.PARAM.INPUT_SENT]
        gv_feat, att_feats, att_mask, p_att_feats, geo_feats, add_feats = self.preprocess(**kwargs)
        ...
        add_feats = utils.expand_tensor(add_feats, cfg.DATA_LOADER.SEQ_PER_IMG) if add_feats is not None else None
        ...
        for t in range(seq.size(1)):
            ...
            kwargs = self.make_kwargs(wt, gv_feat, att_feats, att_mask, p_att_feats, state, geo_feats=geo_feats, add_feats=add_feats)
    ...
    def decode_beam(self, **kwargs):
        ...
        for t in range(cfg.MODEL.SEQ_LEN):
            ...
            if t == 0:
                ...
                add_feats = utils.expand_tensor(add_feats, beam_size) if add_feats is not None else None
