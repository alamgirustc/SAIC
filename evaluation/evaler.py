...
class Evaler(object):
    def __init__(
        ...
        self.eval_loader = data_loader.load_val(eval_ids, gv_feat, att_feats, geo_feats_folder, add_feats_folder)
        self.evaler = evaluation.create(cfg.INFERENCE.EVAL, eval_annfile)

    def make_kwargs(self, indices, ids, gv_feat, att_feats, att_mask, geo_feats, add_feats):
        kwargs = {}
    ...
    def __call__(self, model, rname):
        ...
        results = []
        with torch.no_grad():
            for _, (indices, gv_feat, att_feats, att_mask, geo_feats, add_feats) in tqdm.tqdm(enumerate(self.eval_loader)):
                ...
                kwargs = self.make_kwargs(indices, ids, gv_feat, att_feats, att_mask, geo_feats, add_feats)
