...
class CocoDataset(data.Dataset):
    def __init__(
        ...
        self.add_feats_folder = add_feats_folder if len(add_feats_folder) > 0 else None  # Initialize the add_feats_folder
    ...
    def __getitem__(self, index):
        ...
        if self.add_feats_folder is not None:
            add_feats = np.load(os.path.join(self.add_feats_folder, str(image_id) + '_cnn_feats.npz'))['cnn_features']
            add_feats = np.array(add_feats).astype('float32')
        else:
            add_feats = np.zeros((1, 1))

        if self.seq_len < 0:
            return indices, gv_feat, att_feats, geo_feats, add_feats  # Return add_feats here as well
        ...
        return indices, input_seq, target_seq, gv_feat, att_feats, geo_feats, add_feats  # Return all features correctly
