...
class Attention(nn.Module):
    ...
    # geo_feats -- batch_size * att_num * geo_dim
    def forward(self, h, att_feats, p_att_feats, geo_feats=None, add_feats=None ):
        Wah = self.Wah(h).unsqueeze(1)

        # Concatenate geometric features to the precomputed attention features
        if geo_feats is not None:
            p_att_feats = torch.cat([p_att_feats, geo_feats], dim=-1)
